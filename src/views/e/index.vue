<template>
   <div id="wave" @click="randomColor"></div>
</template>
...
function init() {
   ...
   const material = new THREE.ShaderMaterial({
      uniforms: {
         //设置球的颜色
         color: { value: new THREE.Color(dotColor.value  ) },
      },
      //控制球的大小
      vertexShader:
...
const randomColor = () => {
   ...
   dotColor.value = `hsl(${hue}, 100%, 50%)`
}

watch(dotColor, () => {
   particles.material.uniforms.color.value = new THREE.Color(dotColor.value)
})
onMounted(() => {
   init()
   animate()
